...
/* ----------------------------------------------------------------------
   Contributing author: Shuming Liu
   [ based on dihedral_fourier.cpp Loukas D. Peristeras (Scienomics SARL) ]
------------------------------------------------------------------------- */

#include "improper_dihedral_fourier.h"
#include <mpi.h>
#include <cmath>
#include "atom.h"
#include "comm.h"
...
ImproperDihedralFourier::ImproperDihedralFourier(LAMMPS *lmp) : Improper(lmp)
{
   writedata = 1;
}
...
ImproperDihedralFourier::~ImproperDihedralFourier()
{
  if (allocated) {
...
void ImproperDihedralFourier::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nimproperlist; n++) {
    ...
    for (j=0; j<nterms[type]; j++)
    {
      ...
      if (eflag) eimproper += k[type][j] * p_;
...
void ImproperDihedralFourier::allocate()
{
  allocated = 1;
  int n = atom->nimpropertypes;

  memory->create(nterms,n+1,"improper:nterms");
  k = new double * [n+1];
...
void ImproperDihedralFourier::coeff(int narg, char **arg)
{
  ...
  if (nterms_one < 1)
    error->all(FLERR,"Incorrect number of terms arg for improper coefficients");

  if (2+3*nterms_one < narg)
    error->all(FLERR,"Incorrect number of arguments for improper coefficients");
...
void ImproperDihedralFourier::write_restart(FILE *fp)
{

  fwrite(&nterms[1],sizeof(int),atom->nimpropertypes,fp);
  for(int i = 1; i <= atom->nimpropertypes; i++) {
...
void ImproperDihedralFourier::read_restart(FILE *fp)
{
  allocate();

  if (comm->me == 0)
    fread(&nterms[1],sizeof(int),atom->nimpropertypes,fp);

  MPI_Bcast(&nterms[1],atom->nimpropertypes,MPI_INT,0,world);
...
void ImproperDihedralFourier::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nimpropertypes; i++)
  {
